            ...
            EditText ->
              let switchMode = continue $ s {stateMode = EditTree}
               in case key of
                    ...
                    KBS -> textDo $ dullMDelete . textCursorRemove
                    KDel -> textDo $ dullMDelete . textCursorDelete
            ...
            EditTree ->
              let switchMode = continue $ s {stateMode = EditText}
               in case key of
                    ...
                    KChar 'd' ->
                      mDo $ \tc -> case treeCursorRemoveElem toTextCursor tc of
                        Deleted -> Nothing
                        Updated tc' -> Just tc'
                    KChar 'D' ->
                      mDo $ \tc -> case treeCursorRemoveSubTree toTextCursor tc of
                        Deleted -> Nothing
                        Updated tc' -> Just tc'
