...
milliSmos = do
  ...
  fc <-
    case errOrContents of
      Nothing -> pure $ makeForestCursor toTextCursor (cTree True (Node "" []) NE.:| [])
      Just contents ->
        case Yaml.decodeEither' contents of
          Left err -> die $ "Failed to read tree file: " <> prettyPrintParseException err
          Right tree -> pure $ makeForestCursor toTextCursor (NE.map (cTree True) (tree :: NE.NonEmpty (Tree Text)))
...
milliSmosApp =
  App
    { appDraw = draw,
      ...
      appHandleEvent = handleStateEvent,
      appStartEvent = pure,
      appAttrMap =
        const $
          attrMap
...
drawTextCTree (CNode t cf) =
  vBox [hBox [str "- ", txt t], padLeft defaultPadding (drawCForest cf)]
...
handleStateEvent :: State -> BrickEvent n e -> EventM n (Next State)
handleStateEvent s e = case e of
  VtyEvent ve -> case ve of
    EvKey key mods ->
      let textDOUo :: (TextCursor -> Maybe (DeleteOrUpdate TextCursor)) -> EventM n (Next State)
            ...
            EditForest ->
              let switchMode = continue $ s {stateMode = EditText}
               in case key of
                    ...
                    KChar 'd' ->
                      mDo $ \tc -> case forestCursorRemoveElem toTextCursor tc of
                        Deleted -> Nothing
                        Updated tc' -> Just tc'
                    KChar 'D' ->
                      mDo $ \tc -> case forestCursorRemoveSubTree toTextCursor tc of
                        Deleted -> Nothing
                        Updated tc' -> Just tc'
